...
const TintedHeroImage: React.FC<TintedHeroImageProps> = ({ heroImage, skinName, imageMod, size = 275 }) => {
  return (
    <Box size={size}>
      {/* Base Color tint */}
      <Image position="absolute" style={{ tintColor: tintHexMap[imageMod] }} source={heroImage} size={size} alt={skinName} />
      {/* IMAGE - Shown on top of overlay */}
      <Image position="absolute" style={{ opacity: darknessMap[imageMod] || 0.5 }} source={heroImage} size={size} alt={skinName} />
    </Box>
  );
};
